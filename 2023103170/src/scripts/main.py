...
if __name__ == '__main__':
    ...
    v_feat = torch.as_tensor(v_feat, dtype=torch.float) if has_v else None
    a_feat = torch.as_tensor(a_feat, dtype=torch.float) if has_a else None
    t_feat = torch.as_tensor(t_feat, dtype=torch.float) if has_t else None
    ...
    test_data = np.load('./Data/'+data_path+'/test.npy', allow_pickle=True)
