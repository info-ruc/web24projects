...
class Net(torch.nn.Module):
    def __init__(self, v_feat, a_feat, t_feat, words_tensor, edge_index, batch_size, num_user, num_item, aggr_mode, concate, num_layer, has_id, user_item_dict, reg_weight, dim_x):
        ...
        self.num_modal = 0
        self.v_feat = torch.as_tensor(v_feat,dtype=torch.float)
        ...
        self.a_feat = torch.as_tensor(a_feat,dtype=torch.float)
        ...
        self.t_feat = torch.as_tensor(t_feat,dtype=torch.float)
        ...
        self.id_embedding = nn.init.xavier_normal_(torch.rand((num_user+num_item, dim_x), requires_grad=True))
    ...
    def accuracy(self, step=100, topk=10):
        ...
        while end_index <= self.num_user and start_index < end_index:
            ...
            index_of_rank_list = torch.topk(score_matrix, topk)
            all_index_of_rank_list = torch.cat((all_index_of_rank_list, index_of_rank_list.indices.cpu()+self.num_user), dim=0)
