...
def create_dataset(dataset_type, config):
    ...
    if dataset_type in ["pt_train", "ret_train"]:
        ...
        for m in train_media_types:
            ...
            if config.rtime == True:
                dataset_cls = RtimeFintuneDataset
            elif m == "image":
...
def create_loader(datasets, samplers, batch_size, num_workers, is_trains, collate_fns, train_shuffle):
    loaders = []
    for dataset, sampler, bs, n_worker, is_train, collate_fn in zip(
        datasets, samplers, batch_size, num_workers, is_trains, collate_fns
    ):
        if is_train:
            shuffle = (sampler is None) and train_shuffle
            drop_last = True
        else:
            shuffle = False
            drop_last = False
