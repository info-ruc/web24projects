...
class RtimeVideoBaseDataset(Dataset):
    """Base class that implements the image and video loading methods"""
    ...
    def get_anno(self, index):
        ...
        anno = self.anno_list[index]
        if self.data_root is not None:
            anno["video_pos"] = os.path.join(self.data_root, anno["video_pos"])
            anno["video_neg"] = os.path.join(self.data_root, anno["video_neg"])
        return anno
    ...
    def load_and_transform_media_data_video(self, index, data_path):
        for _ in range(self.num_tries):
            try:
                ...
                index = random.randint(0, len(self) - 1)
                ann = self.get_anno(index)
                data_path = os.path.join(self.data_root, ann["video_pos"])
                continue
            # shared aug for video frames
            frames = self.transform(frames)
            return frames, index
