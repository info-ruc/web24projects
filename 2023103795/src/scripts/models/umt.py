...
from .criterions import MLMLoss, VTC_VTM_Loss, UTA_Loss, Rtime_VTC_VTM_Loss, webvid_VTC_VTM_Loss
from .mask import (
...
class UMT_temporal(nn.Module):
    """docstring for UMT"""

    def __init__(self, config, tokenizer, is_pretrain=True):
        super(UMT_temporal, self).__init__()
        ...
        self.criterion_vtc_vtm = Rtime_VTC_VTM_Loss(config.criterion.vtm_hard_neg)
        self.criterion_mlm = MLMLoss(config.criterion.mlm_masking_prob, tokenizer)

    def forward(self, video, text, pos_num, neg_num):
        """forward and calculate loss.
        ...
        # import pdb; pdb.set_trace()
        B = video.shape[0] // 2
        vision_embeds, pooled_vision_embeds, student_output, clip_output= self.encode_vision(video)
        text_embeds, pooled_text_embeds = self.encode_text(text) # B*(pos+neg), L, C 
        
        N, L, C = text_embeds.shape

        # text_embeds = text_embeds.reshape(N // B, B, L, C).permute(1, 0, 2, 3).reshape(N, L, C)
        # pooled_text_embeds = text_embeds[:, 0]
        

        
        pos_vision_embeds = vision_embeds[:B]
        pos_pooled_vision_embeds = pooled_vision_embeds[:B]
        neg_vision_embeds = vision_embeds[B:]
        neg_pooled_vision_embeds = pooled_vision_embeds[B:]
        
        pos_text_embeds = text_embeds[:B * pos_num]
        pos_pooled_text_embeds = pooled_text_embeds[:B * pos_num]
        neg_text_embeds = text_embeds[B*pos_num:]
        neg_pooled_text_embeds = pooled_text_embeds[B*pos_num:]
        
        # pos_text_proj = pos_text_proj.reshape(pos_num, B, -1).permute(1, 0, 2).reshape(pos_num*B, -1) # B, pos_num, C -> B*pos_num, C
        # neg_text_proj = neg_text_proj.reshape(neg_num, B, -1).permute(1, 0, 2).reshape(neg_num*B, -1)

        # pos_text_proj = pos_text_proj.reshape(pos_num, B, -1).permute(1, 0, 2).reshape(pos_num*B, -1) # B, pos_num, C -> B*pos_num, C
        # neg_text_proj = neg_text_proj.reshape(neg_num, B, -1).permute(1, 0, 2).reshape(neg_num*B, -1)

        vision_proj = self.vision_proj(pooled_vision_embeds)
        pos_vision_proj = vision_proj[:B]
        neg_vision_proj = vision_proj[B:]
    
        text_proj = self.text_proj(pooled_text_embeds) # B*pos_num, C
        pos_text_proj = text_proj[:B*pos_num]
        neg_text_proj = text_proj[B*pos_num:]
        
        manual_pos_text_proj = pos_text_proj[::pos_num]
        manual_neg_text_proj = neg_text_proj[::neg_num]
        manual_pos_text_embeds = pos_text_embeds[::pos_num]
        manual_neg_text_embeds = neg_text_embeds[::neg_num]
        # calculate loss
        ## MCA loss
        if self.loss_weight.uta != 0:
            loss_uta = self.criterion_uta.uta_loss(student_output, clip_output)
        ...
        if self.loss_weight.vtc != 0:
            loss_vtc = self.criterion_vtc_vtm.vtc_loss(
                pos_vision_proj, neg_vision_proj, pos_text_proj, neg_text_proj, B, pos_num, neg_num, self.temp, all_gather=True,
            )
        else:
            loss_vtc = torch.tensor(0)
        ...
        if self.loss_weight.vtm != 0:
            ...
            loss_vtm = self.criterion_vtc_vtm.vtm_loss(
                ...
                self.itm_head,
                self.temp,
                torch.cat([pos_vision_embeds,neg_vision_embeds], dim=0),
                torch.cat([manual_pos_text_embeds,manual_neg_text_embeds], dim=0),
                torch.cat([pos_vision_proj,neg_vision_proj], dim=0),
                torch.cat([manual_pos_text_proj,manual_neg_text_proj], dim=0),
                torch.cat([text.attention_mask[:B*pos_num:pos_num],text.attention_mask[B*pos_num::neg_num]], dim=0),
            )
        else:
            loss_vtm = torch.tensor(0)
...
#         if test:
#             vision_embeds, pooled_vision_embeds, _ = self.vision_encoder(
#                 image, True, use_image, keep_temporal,
#             )
#             return vision_embeds, pooled_vision_embeds
#         else:
#             mask, clip_output = self.encode_teacher(image)
...
class UMT_webvid(nn.Module):
    """docstring for UMT"""

    def __init__(self, config, tokenizer, is_pretrain=True):
        super(UMT_webvid, self).__init__()
        ...
        self.temp = nn.parameter.Parameter(torch.ones([]) * config.model.temp)
        if self.config.criterion.loss_weight.vtm != 0:
            self.itm_head = nn.Linear(self.text_width, 2)
        ...
        self.criterion_vtc_vtm = webvid_VTC_VTM_Loss(config.criterion.vtm_hard_neg)
        self.criterion_mlm = MLMLoss(config.criterion.mlm_masking_prob, tokenizer)
