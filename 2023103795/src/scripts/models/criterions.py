...
class VTC_VTM_Loss(nn.Module):
    ...
    def vtm_loss(
        ...
        # temporal vtm
        # vision_neg_indices = torch.tensor([i ^ 1 for i in range(sim_v2t.shape[0])])
        # txt_neg_indices = torch.tensor([i ^ 1 for i in range(sim_t2v.shape[0])])
...
class Rtime_VTC_VTM_Loss(nn.Module):
    """video-text contrastive and matching losses."""
    ...
    def vtc_loss(
        self,
        pos_vision_proj, neg_vision_proj, pos_text_proj, neg_text_proj, B, pos_num, neg_num,
        temp=1.0,
        all_gather=True,
    ):
        """forward to calculate the loss
        ...
        if all_gather:
            gather_args = self.get_gather_args()
            pos_vision_proj = allgather_wgrad(pos_vision_proj, gather_args) # (ngpu * bsz, L, C)
            neg_vision_proj = allgather_wgrad(neg_vision_proj, gather_args)
            pos_text_proj = allgather_wgrad(pos_text_proj, gather_args) #(ngpu*B*pos_num, C)
            neg_text_proj = allgather_wgrad(neg_text_proj, gather_args)
            # if idx is not None:
            #     idx = allgather_wgrad(idx, gather_args)
        # print(idx)
        # exit(0)
        sim_v2t, sim_t2v = get_sim(torch.cat([pos_vision_proj, neg_vision_proj], dim=0), torch.cat([pos_text_proj, neg_text_proj], dim=0), temp) # (B + B) * (B*posnum + B*negnum)
        # import pdb; pdb.set_trace()
        sum_B = get_world_size() * B
        nce_v2t = sim_v2t
        nce_t2v = torch.cat([sim_t2v[:sum_B * pos_num:pos_num], sim_t2v[sum_B * pos_num::neg_num]], dim=0)
        # import pdb; pdb.set_trace()
        with torch.no_grad():
            sim_v2t_targets = torch.zeros(nce_v2t.shape, dtype=nce_v2t.dtype)
            sim_t2v_targets = torch.zeros(nce_t2v.shape, dtype=nce_t2v.dtype)

            for i in range(sum_B):
                sim_v2t_targets[i, i * pos_num:(i + 1) * pos_num] = torch.tensor(1.0, dtype=torch.float16)
            for i in range(sum_B, 2 * sum_B):
                sim_v2t_targets[i, (sum_B * pos_num + (i - sum_B) * neg_num):(sum_B * pos_num + (i + 1 - sum_B) * neg_num)] = torch.tensor(1.0, dtype=torch.float16)
                
            sim_t2v_targets.fill_diagonal_(torch.tensor(1.0, dtype=torch.float16))

            sim_t2v_targets = sim_t2v_targets.cuda()
            sim_v2t_targets = sim_v2t_targets.cuda()
            # import pdb; pdb.set_trace()
            # sim_v2t_targets = self.get_mask(sim_v2t, idx=idx, normalize=True)
            # sim_t2v_targets = sim_v2t_targets

        loss_i2t = -torch.sum(F.log_softmax(nce_v2t, dim=1) * sim_v2t_targets, dim=1).mean()
        loss_t2i = -torch.sum(F.log_softmax(nce_t2v, dim=1) * sim_t2v_targets, dim=1).mean()
    ...
    def vtm_loss(
        ...
        # temporal vtm
        B = sim_v2t.shape[0] // 2
        vision_neg_indices = torch.tensor([(i + B) % (2 * B) for i in range(sim_v2t.shape[0])])
        txt_neg_indices = torch.tensor([(i + B) % (2 * B) for i in range(sim_t2v.shape[0])])
...
class webvid_VTC_VTM_Loss(nn.Module):
    """video-text contrastive and matching losses."""
    ...
    def vtc_loss(
        ...
        """
        bsz = vision_proj.shape[0]
        pos_proj = text_proj[:bsz]
        neg_proj = text_proj[bsz:]
        if all_gather:
            gather_args = self.get_gather_args()
            vision_proj = allgather_wgrad(vision_proj, gather_args)
            pos_proj = allgather_wgrad(pos_proj, gather_args)
            neg_proj = allgather_wgrad(neg_proj, gather_args)
            if idx is not None:
                idx = allgather_wgrad(idx, gather_args)
        # print(idx)
        # exit(0)
        pos_v2t, pos_t2v = get_sim(vision_proj, pos_proj, temp)
        neg_v2t, neg_t2v = get_sim(vision_proj, neg_proj, temp)
        # sim_v2t, sim_t2v = get_sim(vision_proj, text_proj, temp)
        sim_v2t = torch.cat([pos_v2t, neg_v2t], dim=1)
        sim_t2v = pos_t2v
        with torch.no_grad():
            sim_v2t_targets = self.get_mask(sim_v2t, idx=None, normalize=True)
            sim_t2v_targets = self.get_mask(sim_t2v, idx=idx, normalize=True)
        # import pdb; pdb.set_trace()
        loss_i2t = -torch.sum(F.log_softmax(sim_v2t, dim=1) * sim_v2t_targets, dim=1).mean()
        loss_t2i = -torch.sum(F.log_softmax(sim_t2v, dim=1) * sim_t2v_targets, dim=1).mean()
