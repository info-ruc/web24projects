...
from .criterions import FrameNCEContrastiveLoss
class vidlp(nn.Module):
    def __init__(self, config) -> None:
        super(vidlp, self).__init__()

        # <=== Start of CLIP Encoders
        clip_state_dict = CLIP.get_config(config.model.vision_encoder.name)
    ...
    def forward(self, video, text):
        sequence_output = self.forward_text(text)
        visual_output = self.forward_video(video)
        sim_matrix, v2t = self.sim_score(sequence_output, visual_output)
        ...
        return dict(loss_vtc=loss)


    def forward_video(self, video):
        # video shape here should be (B, T, C, H, W)
        B, T, C, H, W = video.shape
        video = video.view(-1, C, H, W)
        visual_hidden = self.clip.encode_image(video, video_frame=T).float()  # shape=(B*T,L,D)
        visual_hidden = visual_hidden.view(-1, T, visual_hidden.size(-2),
        ...
        if self.training:
            ...
            select_idx = random_idx[:select_num-1] + 1
        ...
        # visual_hidden here is (B, T, S, D)
        video_mask = torch.ones(visual_hidden.size(0), visual_hidden.size(1))
        visual_output = visual_hidden + self.temporal_transformer(visual_hidden, video_mask)
        
        return visual_output[:, :, 0, :]
    
    def forward_text(self, input_ids):
        sequence_output = self.clip.encode_text(input_ids).float() # shape here is (B, D)

        return sequence_output

    def sim_score(self, sequence_output, visual_output):
        sequence_output = F.normalize(sequence_output, dim=1) # shape here is (B, D)
        visual_output = F.normalize(visual_output, dim=1) # shape here is (B, T, D)
        sim_matrix = torch.einsum("nk,mjk->njm", sequence_output, visual_output)
        sim_matrix_weight = self.softmax(4 * sim_matrix, dim=1)
        sim_matrix = sim_matrix_weight * sim_matrix
        sim_matrix = torch.sum(sim_matrix, dim=1)
...
# Below are borrow from openai clip
_MODELS = {
