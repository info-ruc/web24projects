...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        B, N, C = x.shape # N = frame * (224 / 16) ^ 2
        qkv_bias = None
        ...
        qkv = qkv.reshape(B, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4) # 3, B, numhead, N, C
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple) B, numhead, N, c

        q = q * self.scale
        attn = (q @ k.transpose(-2, -1)) # B, numhead, N, N 
        # add temporal mask at here
        if mask is not None:
            # import pdb; pdb.set_trace()
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, -1) # B, numhead*N, c
        x = self.proj(x) # B, N, c
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # temporal_umt
        # self.attn_temporal_mask = Attention(
        #     dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
        #     attn_drop=attn_drop, proj_drop=drop, attn_head_dim=attn_head_dim)
        ...
        # temporal_umt
        # self.lora_r = nn.Parameter(torch.ones((dim)), requires_grad=True)
        if init_values > 0:
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x, **kwargs):
        B, C, T, H, W = x.shape # B, 3, T, 224, 224
        # FIXME look at relaxing size constraints
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x).flatten(2).transpose(1, 2) # B, 196*T, 768
        return x
...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def forward_features(self, x, mask=None, use_image=False):
        ...
        B, _, C = x.shape # _ = frame * L = 
        # import pdb; pdb.set_trace()
        temporal_mask = None # change temporal mask at here
        # temporal_mask = torch.triu(torch.ones(_, _), diagonal=1).bool().to('cuda')
        if mask is not None:
        ...
        for idx, blk in enumerate(self.blocks):
            if self.use_checkpoint and idx < self.checkpoint_num:
                x_vis = checkpoint.checkpoint(blk, x_vis, temporal_mask)
            else:
                x_vis = blk(x_vis, temporal_mask)
