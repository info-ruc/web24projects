...
def itm_eval(scores_i2t, scores_t2i, txt2img, img2txt):
    # np.save("/data2/dy/code/unmasked_teacher/multi_modality/exp/finetuning/ret_rtime/b16_25m_200k/scorei2t.npy", scores_i2t)
    # np.save("/data2/dy/code/unmasked_teacher/multi_modality/exp/finetuning/ret_rtime/b16_25m_200k/scoret2i.npy", scores_t2i)
    # length = scores_i2t.shape[0]
    # scores_i2t = scores_i2t[:length // 2, :length // 2]
    # scores_t2i = scores_t2i[:length // 2, :length // 2]
    # Images->Text
    ...
    if len(ranks) % 2 == 0:
        for index, score in enumerate(scores_i2t):
            ranks[index] = int(score[index] > score[index ^ 1])
    txt_acc = 100.0 * sum(ranks) / len(ranks)

    ranks = np.zeros(scores_t2i.shape[0])
    if len(ranks) % 2 == 0:
        for index, score in enumerate(scores_t2i):
            ranks[index] = int(score[index] > score[index ^ 1])
    img_acc = 100.0 * sum(ranks) / len(ranks)
