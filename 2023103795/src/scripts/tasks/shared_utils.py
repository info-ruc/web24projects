...
def setup_model(
    ...
    if "bert" in config.model.text_encoder.name:
        tokenizer = BertTokenizer.from_pretrained(config.model.text_encoder.pretrained, proxies={
    ...
    #     if "lora_r" not in name and "attn_temporal_mask" not in name and "vision_proj" not in name:
    ...
    if osp.isfile(config.pretrained_path):
        ...
        if config.resume:
            ...
            if not config.evaluate or config.get("zero_shot", False):  # finetuning from a pretarined weights. |evaluate == Fasle || zero_shot == True
                # import pdb; pdb.set_trace()
        ...
        # for name, param in model_without_ddp.named_parameters():
        #     if "attn_temporal_mask" in name:
        #         # print(param)
        #         replace_name = name.replace("attn_temporal_mask", "attn")
        #         param.data = state_dict[replace_name].cuda().data
        #         param.requires_grad = True

        # for name, param in model_without_ddp.named_parameters():
        #     if "lora_r" not in name and "attn_temporal_mask" not in name:
        #         # print(param)
        #         # import pdb; pdb.set_trace()
        #         pass
        #     else:
