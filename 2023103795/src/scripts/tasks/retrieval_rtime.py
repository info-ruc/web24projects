...
from models.umt import UMT, UMT_temporal
...
def train(
    ...
    # model.module.vision_encoder.encoder.pos_embed = model.module.vision_encoder.encoder.img_pos_embed.repeat(1, config.num_frames, 1)


    for i, (media_type, (video_pos, video_neg, text_pos, text_neg, reverse)) in enumerate(iterator):
        # import pdb; pdb.set_trace()
        video_pos = video_pos.to(device, non_blocking=True)
        video_neg = video_neg.to(device, non_blocking=True)
        video = torch.cat([video_pos, video_neg], dim=0)
        # import pdb; pdb.set_trace()
        # # if text_pos.shape
        # text_pos (pos_num, batchsize)
        # import pdb; pdb.set_trace()
        B = video_pos.shape[0]
        sample_text_pos = []
        for batch in range(B):
            for num in range(config.pos_num):
                sample_text_pos.append(text_pos[num][batch])
        text_pos = sample_text_pos

        sample_text_neg = []
        for batch in range(B):
            for num in range(config.neg_num):
                sample_text_neg.append(text_neg[num][batch])
        text_neg = sample_text_neg
        

        # text_pos = [item for batch in text_pos[:config.pos_num] for item in batch]
        # text_neg = [item for batch in text_neg[:config.neg_num] for item in batch]
         # import pdb; pdb.set_trace()
        ...
        with torch.cuda.amp.autocast(enabled=config.fp16):
            loss_dict = model(video, text_input, config.pos_num, config.neg_num)
            # import pdb;pdb.set_trace()
...
def main(config):
    ...
    # # cancel temporal pos embed
    # model_without_ddp.vision_encoder.encoder.pos_embed = torch.zeros(model_without_ddp.vision_encoder.encoder.pos_embed.shape)
    # model.module.vision_encoder.encoder.pos_embed = model.module.vision_encoder.encoder.img_pos_embed.repeat(1, config.num_frames, 1)
    # print(model_without_ddp.vision_encoder.encoder.pos_embed.requires_grad)
    # exit(0)
    # model_without_ddp.vision_encoder.encoder.pos_embed.requires_grad = False
