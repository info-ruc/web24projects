...
def train(
    ...
    for i, (media_type, (image, text, idx)) in enumerate(iterator):
        ...
        text_input = torch.tensor(tokenizer(
            text
        )).to(device)

        with torch.cuda.amp.autocast(enabled=config.fp16):
            loss_dict = model(image, text_input)
...
def main(config):
    ...
    model_cls = eval(config.model.get('model_cls', 'vidlp'))
    (
        model,
        model_without_ddp,
        optimizer,
