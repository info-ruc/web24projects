...
from opencood.models.sub_modules.cls_weight_bev import cls_weights
...
from opencood.utils.BSAF_utils import similarity,CBAM
...
class CamEncode(nn.Module):  # 提取图像特征进行图像编码
    ...
    def get_eff_features(self, x):  # 使用efficientnet提取特征
        ...
        # Head  endpoints:mlv feature maps
    ...
    def forward(self, x, depth):
        ...
        if self.use_gt_depth:
            ...
            if self.depth_supervision:
                ...
                # return depth_logit,  x_img, new_x
            else:
                return None, None, depth_gt, x_img, new_x
...
class mulBEVfusion(nn.Module):
    def __init__(self,fusion_args):
        super(mulBEVfusion,self).__init__()
        args = fusion_args['args']
        inC = args['in_channels']
        self.outC = args['outC']
        self.discrete_ratio = args['voxel_size'][0]
        self.downsample_rate = 1
        self.conv = nn.Conv2d(inC, 128, kernel_size=3, stride=2, padding=1)
    def forward(self, x, record_len, pairwise_t_matrix):
        ...
        # t = BSFusion(self.outC)
        t = AttFusion(self.outC)

        res = t(res,record_len,pairwise_t_matrix)
...
class BevEncodeMSFusion(nn.Module):
    ...
    def __init__(self, fusion_args):  # inC: 64  outC: not 1 for object detection
        ...
        # elif fusion_args['core_method'] == "weight":
        #     self.fuse_module = [BSFusion(),BSFusion(),BSFusion()]
        else:
...
class ClsFusion(nn.Module):
    def __init__(self):
        super(ClsFusion, self).__init__()

    def forward(self, x, record_len, pairwise_t_matrix,bev_binary):
        _, C, H, W = x.shape
        B, L = pairwise_t_matrix.shape[:2]
        split_x = regroup(x, record_len)
        batch_node_features = split_x
        ...
        for b in range(B):
            ...
            neighbor_feature = warp_affine_simple(batch_node_features[b],
                                                  t_matrix[i, :, :, :],
                                                  (H, W))
            # bev_binary: H,W,1     neighbor_feature:record_len,H,W,C
            bev_binary = bev_binary.view(1,1,H,W).repeat(record_len,C,1,1)
            neighbor_feature = bev_binary * neighbor_feature
            out.append(torch.sum(neighbor_feature, dim=0,keepdim=True)[0])
        out = torch.stack(out)
...
class BSFusion(nn.Module):
    def __init__(self,inC):
        super(BSFusion, self).__init__()
        self.att = ScaledDotProductAttention(inC)
    def forward(self, x, record_len, pairwise_t_matrix):
        _, C, H, W = x.shape
        B, L = pairwise_t_matrix.shape[:2]
        split_x = regroup(x, record_len)
        ...
        bev_weight = []
        batch_bev_weight = []
        new_slice = torch.ones((1, H, W, 1), dtype=x.dtype, device=x.device)
        # iterate each batch
        for b in range(B):
            N = record_len[b]
            if N == 1:
                ...
                for n in range(N):
                    if n==0:
                        ego = neighbor_feature[n]
                        x = ego.view(1, C, -1).permute(2, 0, 1)
                        ego_attention = self.att(x,x,x).view(H,W,-1)
                        ego = ego.permute(1,2,0)
                        continue
                    else:
                        neighbor = neighbor_feature[n]
                        x = neighbor.view(1, C, -1).permute(2, 0, 1)
                        neighbor_attention = self.att(x, x, x).view( H, W, -1)
                        neighbor = neighbor.permute(1,2,0)
                    # 1 , H , W , 1
                    s = similarity(ego,neighbor)
                    rat = torch.sum(torch.div(neighbor_attention,ego_attention),dim=-1).unsqueeze(-1)
                    weight = torch.mul(s,rat)
                    bev_weight.append(weight)
                # N-1,H,W,1
                bev_weight = torch.stack(bev_weight).squeeze(1)
                bev_weight = F.softmax(bev_weight,dim=0)
                # N, H ,W ,1
                bev_weight = torch.cat([bev_weight, new_slice], dim=0)
                batch_bev_weight.append(bev_weight)
        out = torch.stack(out)
        # bs , N ,H , W , 1
        if len(batch_bev_weight) != 0:
            batch_bev_weight = torch.stack(batch_bev_weight)
            batch_bev_weight = batch_bev_weight.expand(-1, -1, -1, -1 , C)  # shape becomes [1, 2, 100, 100 , 64]
            batch_bev_weight = batch_bev_weight.permute(0,1,4,2,3)
            out = out * batch_bev_weight + out
        # out = torch.mean(out,dim=1).squeeze(1)
        # 1, 64 , 120 , 120
        out = torch.sum(out, dim=1).squeeze(1)
        return out

class BevEncodeClsFusion(nn.Module):
    """
    Multiscale version of ResNet Encoder
    """
    def __init__(self, fusion_args):  # inC: 64  outC: not 1 for object detection
        super(BevEncodeClsFusion, self).__init__()
        args = fusion_args['args']
        inC = args['in_channels'] #256
        self.camC = inC  # 图像特征维度
        self.discrete_ratio = args['voxel_size'][0]
        self.downsample_rate = 1
        # 使用resnet的前3个stage作为backbone
        trunk = resnet18(pretrained=False, zero_init_residual=True)
        ...
        self.cls_heads = {
            64: self._make_head(64).to('cuda'),
            128: self._make_head(128).to('cuda'),
            256: self._make_head(256).to('cuda')
        }
        if fusion_args['core_method'] == "max_ms":
            self.fuse_module = [MaxFusion(), MaxFusion(), MaxFusion()]
        elif fusion_args['core_method'] == "att_ms":
            self.fuse_module = [AttFusion(64), AttFusion(128), AttFusion(256)]
        elif fusion_args['core_method'] == "cls_ms":
            self.fuse_module = [ClsFusion(),ClsFusion(),ClsFusion()]
        else:
            raise "not implemented"

    def _make_head(self, in_channels):
        return nn.Sequential(
            nn.Conv2d(in_channels, 32, kernel_size=1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, 1, kernel_size=1, padding=0, bias=True)
        )

    def cls_info_weight(self,x,record_len, pairwise_t_matrix):
        _,C,H,W = x.shape
        bev_cls_map_2d = self.cls_heads[C](x)
        bev_cls_map_2d = bev_cls_map_2d.sigmoid()
        bev_binary = cls_weights(bev_cls_map_2d, record_len, pairwise_t_matrix)
        return bev_binary
    ...
    def forward(self, x, record_len, pairwise_t_matrix):  # x: 4 x 64 x 240 x 240
        ...
        # add cls ?
        x1 = self.layer1(x)  # x1: 4 x 64 x 120 x 120
        cls1 = self.cls_info_weight(x1,record_len,pairwise_t_matrix)
        x2 = self.layer2(x1)  # x2: 4 x 128 x 60 x 60
        cls2 = self.cls_info_weight(x2, record_len, pairwise_t_matrix)
        x3 = self.layer3(x2)  # x3: 4 x 256 x 30 x 30
        cls3 = self.cls_info_weight(x3, record_len, pairwise_t_matrix)
        x_single = self.down_layer(self.up_layer1(self.up_layer2(x3, x2), x1)) # 4 x 64 x 120 x 120

        x1_fuse = self.fuse_module[0](x1, record_len, pairwise_t_matrix, cls1)
        x2_fuse = self.fuse_module[1](x2, record_len, pairwise_t_matrix, cls2)
        x3_fuse = self.fuse_module[2](x3, record_len, pairwise_t_matrix, cls3)
