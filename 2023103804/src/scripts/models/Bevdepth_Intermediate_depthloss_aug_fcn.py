...
class BevdepthIntermediatedepthlossaugfcn(LiftSplatShoot):
    def __init__(self, args):
        super(BevdepthIntermediatedepthlossaugfcn, self).__init__(args)
    ...
    def depthforward(self,x,rots,trans,intrins,post_rots,post_trans,transform_matrix,lidar_depth,depth_downsample):
        ...
        attention_mask_car = cls_img[:, 1, :, :]  # [N, H, W]
        attention_mask_non_car = cls_img[:, 0, :, :]  # [N, H, W]

        # visualize for mask
        # data1 = attention_mask_car[0].detach().cpu().numpy()
        # data2 = attention_mask_non_car[0].detach().cpu().numpy()
        # data3 = x[0][0].permute(1,2,0).detach().cpu().numpy()
        # # 显示 data1
        # plt.figure()
        # sns.heatmap(data1, cmap="Reds")
        # plt.axis('off')
        # plt.savefig('/home/fulongtai/CoCa3D/opencood/figures/carmask', transparent=False, dpi=400)
        # plt.show()
        #
        # # 显示 data2
        # plt.figure()
        ...
        # plt.axis('off')
        # plt.savefig('/home/fulongtai/CoCa3D/opencood/figures/nocarmask', transparent=False, dpi=400)
        # plt.show()
        #
        # # 显示 data3
        # plt.figure()
        # plt.imshow(data3, cmap='gray')  # 假设data3是灰度图
        # plt.axis('off')
        # plt.savefig('/home/fulongtai/CoCa3D/opencood/figures/image', transparent=False, dpi=400)
        # plt.show()
        # plt.close()


        # Expand dimensions to match depth_map shape [N, 1, H, W]
        attention_mask_car = attention_mask_car.unsqueeze(1)
        attention_mask_non_car = attention_mask_non_car.unsqueeze(1) * 0.5
        # Apply attention masks
        depth_map_car = depth * attention_mask_car
        depth_map_non_car = depth * attention_mask_non_car
        # For simple addition:
        final_depth = depth_map_car + depth_map_non_car

        # depth = torch.cat([depth, cls_img], dim=1)
        # depth = nn.Conv2d(self.D+1, self.D, kernel_size=1)(depth)
        ...
        return x,depth_logit,final_depth,get_bev_depth
    ...
    def _forward_ms(self, data_dict):
        ...
        # depth_logit = depth_logit.permute(0,2,3,1).reshape(-1,self.D)
...
def compile_model(grid_conf, data_aug_conf, outC):
    return BevdepthIntermediatedepthlossaugfcn(grid_conf, data_aug_conf, outC)