...
from .sub_modules.homography import clsdepthfusion, mulclsdepthfusion
...
class BevdepthIntermediatedepthlossaugfcnfusionmultifg(LiftSplatShoot):
    def __init__(self, args):
        super(BevdepthIntermediatedepthlossaugfcnfusionmultifg, self).__init__(args)
        ...
        self.clsdepthfusion = mulclsdepthfusion()
        self.fcn = FCN8s(2)
    ...
    def depthforward(self,x,rots,trans,intrins,post_rots,post_trans,transform_matrix,lidar_depth,depth_downsample):
        ...
        if num_channels == 4:
            img_feats,cls_img,fg = self.get_cam_feats(x)
        else:
        ...
        # depth_gt_fg = fg * depth_downsample
        ...
        # vis
        # max_depth_indices = torch.argmax(final_depth, dim=1)
        # # 将索引转化为真实深度值
        # depth_values_corrected = max_depth_indices * mask_not_47
        # data1 = depth_values_corrected[0].detach().cpu().numpy()
        # plt.figure()
        ...
        #
        # plt.close()
        return x, depth_logit, final_depth, get_bev_depth, cls_img_3d,cls_img,fg
    ...
    def get_cam_feats(self, x):
        ...
        if(C == 4):
            fg = x[:,3:4,:,:]
            fg = F.max_pool2d(fg, self.downsample, self.downsample)
            fg = fg.reshape(-1,fg.shape[2], fg.shape[3])
            x = x[:,:3,:,:]
    ...
    def _forward_ms(self, data_dict):
        ...
        bev_map, depth_logit, depth_pred,get_bev_depth,cls_img_3d,cls_img,fg = self.depthforward(x,rots,trans,intrins,post_rots,post_trans,transform_matrix,lidar_depth,depth_downsample)   # cls_img: N,2,240,240
        ...
        depth_item = [depth_logit,lidar_depth,fg]
        output_dict = {'psm': psm,
                       'rm': rm,
...
def compile_model(grid_conf, data_aug_conf, outC):
    return BevdepthIntermediatedepthlossaugfcnfusionmultifg(grid_conf, data_aug_conf, outC)