...
class BevdepthIntermediatemlcls(LiftSplatShoot):
    def __init__(self, args):
        super(BevdepthIntermediatemlcls, self).__init__(args)

        fusion_args = args['fusion_args']
        self.ms = args['fusion_args']['core_method'].endswith("ms")
        if self.ms:
            self.bevencode = BevEncodeClsFusion(fusion_args)
        else:
    ...
    def fill_depth(self,depth, prob_map):
        b,N, H, W = depth.size()
        ...
        # 获取有深度值的像素的位置和值
        known_depth_coords = torch.nonzero(depth_downsampled, as_tuple=False)
        known_depth_values = depth_downsampled[
            known_depth_coords[:, 0], known_depth_coords[:, 1], known_depth_coords[:, 2]]

        # 获取缺失深度值的像素的位置
        unknown_depth_coords = torch.nonzero(depth_downsampled == 0, as_tuple=False)
    ...
    def depthforward(self,x,rots,trans,intrins,post_rots,post_trans,transform_matrix,lidar_depth):
        ...
        return x,[depth,depth_gt_indices],depth_gt
    ...
    def _forward_ms(self, data_dict):
        ...
        bev_map, depth_items,depth_gt  = self.depthforward(x,rots,trans,intrins,post_rots,post_trans,transform_matrix,lidar_depth)
...
def compile_model(grid_conf, data_aug_conf, outC):
    return BevdepthIntermediatemlcls(grid_conf, data_aug_conf, outC)