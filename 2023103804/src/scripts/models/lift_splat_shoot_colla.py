...
class LiftSplatShootColla(LiftSplatShoot):
    ...
    def get_cam_feats(self, x,depth):
        ...
        """
        # B: 4  N: 4  C: 3  imH: 256  imW: 352
        B, N, C, imH, imW = x.shape

        x = x.view(B*N, C, imH, imW)  # B和N两个维度合起来  x: 16 x 4 x 256 x 352
        depth_logit, depth_gt_indices, depth_gt, ori_x, x = self.camencode(x,depth) # 进行图像编码 ori_x: B*N x C x fH x fW, x: B*N x C x D x fH x fW(24 x 64 x 41 x 16 x 22)
    ...
    def _forward_ms(self, data_dict):
        ...
        x, _, depth_items, depth_items_global = self.get_cam_feats(x,lidar_depth)  # 提取图像特征并预测深度编码 x: B x N x D x fH x fW x C(4 x N x 42 x 16 x 22 x 64)
