# -*- coding: utf-8 -*-
# Author: Hao Xiang <haxiang@g.ucla.edu>, Yue Hu <phyllis1sjtu@outlook.com>
# License: TDG-Attribution-NonCommercial-NoDistrib

"""
Implementation of V2VNet Fusion with Graph Convolutional Network (GCN)
"""
...
class GCNFusion(nn.Module):
    def __init__(self, feature_dim, out_dim):
        ...
        self.gcn1 = GCNLayer(feature_dim, 128)
...
class TransformerFusion(nn.Module):
    ...
    def forward(self, batch_neighbor_feature, batch_neighbor_feature_pe, batch_confidence_map, record_len):
        ...
        for b in range(B):
            ...
            neighbor_feature_flat = neighbor_feature.view(N, C, H * W)

            if self.with_spe:
                neighbor_feature_pe = batch_neighbor_feature_pe[b]
                neighbor_feature_flat_pe = neighbor_feature_pe.view(N, C, H * W)
                query = neighbor_feature_flat_pe[0:1, ...].permute(0, 2, 1)
                key = neighbor_feature_flat_pe.permute(0, 2, 1)
            else:
                query = neighbor_feature_flat[0:1, ...].permute(0, 2, 1)
                key = neighbor_feature_flat.permute(0, 2, 1)

            value = neighbor_feature_flat.permute(0, 2, 1)

            if self.with_scm:
                confidence_map = batch_confidence_map[b]
                fused_feature = self.encode_layer(query, key, value, confidence_map)
            else:
...
class Where2comm(nn.Module):
    def __init__(self, args):
        ...
        self.agg_mode = args['agg_operator']
        ...
        if self.multi_scale:
            ...
            for idx in range(self.num_levels):
                if self.agg_mode == 'GCN':
                    fuse_network = GCNFusion(num_filters[idx], num_filters[idx])
            ...
            if self.agg_mode == 'GCN':
                self.adj = args['adj']
                self.fuse_modules = GCNFusion(args['agg_operator']['feature_dim'], args['agg_operator']['feature_dim'])
            elif self.agg_mode == 'Transformer':
        ...
        if self.multi_scale:
            ...
            for i in range(self.num_levels):
                ...
                if i == 0:
                    if self.communication:
                        ...
                        _, communication_masks, communication_rates = self.naive_communication(batch_confidence_maps,
                                                                                               record_len,
                                                                                               pairwise_t_matrix)
                ...
                for b in range(B):
                    ...
                    if self.agg_mode == 'GCN':
                        adj = self.adj
                        neighbor_feature = neighbor_feature.view(N, C, H * W).permute(2, 0, 1)  # (H*W, N, C)
                        neighbor_feature = self.fuse_modules[i](neighbor_feature, adj)
                        neighbor_feature = neighbor_feature.permute(1, 2, 0).view(N, C, H, W)
            ...
            for b in range(B):
                ...
                if self.agg_mode == 'GCN':
                    ...
                    neighbor_feature = neighbor_feature.view(N, C, H * W).permute(2, 0, 1)  # (H*W, N, C)
                    neighbor_feature = self.fuse_modules(neighbor_feature, adj)
                    neighbor_feature = neighbor_feature.permute(1, 2, 0).view(N, C, H, W)
                    x_fuse.append(neighbor_feature[0])
                else:
