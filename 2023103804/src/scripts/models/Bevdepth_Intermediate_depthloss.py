...
class BevdepthIntermediatedepthloss(LiftSplatShoot):
    def __init__(self, args):
        super(BevdepthIntermediatedepthloss, self).__init__(args)
    ...
    def _forward_ms(self, data_dict):
        ...
        # data = depth_pred[0].permute(1,2,0).detach().cpu().numpy()
        # max_data = data.max(axis=2)
        # plt.imshow(max_data, cmap='jet')
        # plt.colorbar()
        # plt.show()
...
def compile_model(grid_conf, data_aug_conf, outC):
    return BevdepthIntermediatedepthloss(grid_conf, data_aug_conf, outC)