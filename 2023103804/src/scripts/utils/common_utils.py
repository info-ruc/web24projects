...
def get_true_lidar2img_matrix(xx, lidar2img_matrix,record_len):
    '''
    lidar2img_matrix: tensor (B,max_cav,num_cam, 1, 1)
    '''
    B, max_N , N_cam, _,_ = lidar2img_matrix.shape
    # split_x = regroup(xx, record_len)
    # batch_node_features = split_x
    out = []
    for b in range(B):
        N = record_len[b] #true n_agent
        li_matrix = lidar2img_matrix[b][:N, :N_cam, :, :]
        # update each node i
        out.append(li_matrix)
