...
class depthCameraIntermediateFusionDatasetml(camera_basedataset.CameraBaseDataset):
    """
    ...
    def __init__(self, params, visualize, train=True):
        super(depthCameraIntermediateFusionDatasetml, self).__init__(params, visualize, train)
    ...
    def get_item_single_car_camera(self, selected_cav_base, ego_cav_base):
        ...
        for idx, img in enumerate(camera_data_list):
            ...
            # used for lidar project to camera
            ego_camera = x1_to_x2(ego_pose_clean,camera_coords)
            ego_camera = np.array(
            ...
            # point_depth = self.get_lidar_depth(rot,tran,intrin,img,lidar_np)
                ...
                "image_inputs":
                    {
                        ...
                        "rots_1": torch.stack(rots_1),
                        "trans_1": torch.stack(trans_1),
    ...
    def __getitem__(self, idx):
        ...
        transformation_matrix_tp = []
        for i, cav_id in enumerate(cav_id_list):
            selected_cav_base = base_data_dict[cav_id]
            # this only get camera ont lidar   for get lidar
            ...
            transformation_matrix_tp.append(selected_cav_processed['transformation_matrix'])
        ...
        # handle point for depth gt
        lidar_one = []
        for j, cav_id in enumerate(cav_id_list):
            # transform the cloud in the same coor
            # filter lidar
            selected_cav_base = base_data_dict[cav_id]
            lidar_np = selected_cav_base['lidar_np']
            lidar_np = shuffle_points(lidar_np)
            ...
            projected_lidar = \
            box_utils.project_points_by_matrix_torch(lidar_np[:, :3],
                                                     transformation_matrix_tp[j])
            lidar_np = projected_lidar[:,:3]
            # Need cloud points
            lidar_np = mask_points_by_range(lidar_np,
                                        self.params['preprocess'][
                                            'cav_lidar_range'])
            # every agent has different number of points.
            lidar_cam = []
            for n in range (self.num_cam):
                rot = agents_image_inputs[0]['rots_1'][n]
                intrin = agents_image_inputs[0]['intrins'][n]  # ego
                tran = agents_image_inputs[0]['trans_1'][n]
                #Todo: check if rot and tran are correct ?
                point_depth = self.get_lidar_depth(rot,tran,intrin,lidar_np)
                # data augmentation
                resize, resize_dims, crop, flip, rotate = sample_augmentation(
                    self.data_aug_conf, self.train
                )
                point_depth_augmented = depth_transform(
                    point_depth, resize, self.data_aug_conf['final_dim'],
                    crop, flip, rotate)
                lidar_cam.append(point_depth_augmented)
            ...
            lidar_cam = torch.stack(lidar_cam)
            lidar_one.append(lidar_cam)
        lidar_one = torch.stack(lidar_one)
        lidar_one = torch.max(lidar_one,dim=0)[0]
        # d_img= agents_image_inputs[0]['imgs'][0].numpy()
        # plt.imshow(d_img.transpose(1, 2, 0))
        # plt.axis('off')  # 不显示坐标轴
        # plt.show()
        # data = lidar_one.numpy()
    ...
    def get_lidar_depth(self, rot,tran,intrins,lidar):
        ...
        # # + renormalization).
        rotated_point_cloud = rotated_point_cloud.type(torch.float32)
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            depth_group = ego_dict['depth_group']
