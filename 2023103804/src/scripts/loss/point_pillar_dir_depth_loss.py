...
class PointPillarDirDepthLoss(nn.Module):
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        if f"depth_items{suffix}" in output_dict and output_dict[f'depth_items{suffix}'] is not None:
            ...
            # depth_gt:(N,48)  depth_pred:(N',D,H,W)
            #TODO: there are some problem ?
            # depth_labels = output_dict[f'depth_items{suffix}'][0]
            # depth_preds = output_dict[f'depth_items{suffix}'][1]
            # fg_mask = torch.max(output_dict[f'depth_items{suffix}'][0], dim=1).values > 0.0
            #
            # # print("Shape of fg_mask:", fg_mask.shape)
            # # print("Sum of fg_mask:", fg_mask.sum().item())
            #
            # depth_loss = (F.binary_cross_entropy_with_logits(
            #     depth_preds[fg_mask],
            #     depth_labels[fg_mask],
            #     reduction='none',
            # ).sum() / max(1.0, fg_mask.sum().item()))
