...
class PointPillarDirDepthbevLoss(nn.Module):
    def __init__(self, args):
        super(PointPillarDirDepthbevLoss, self).__init__()
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        if f"depth_items{suffix}" in output_dict and output_dict[f'depth_items{suffix}'] is not None:
            depth_labels = output_dict[f'depth_items{suffix}'][0]
            depth_preds = output_dict[f'depth_items{suffix}'][1].squeeze(1)
            mask = (depth_labels > 0).float()
            # 计算掩码的MSE损失
            mse_loss_masked = torch.sum(mask * (depth_preds - depth_labels) ** 2) / torch.sum(mask)
            depth_loss = 3.0 * mse_loss_masked
