...
model = transformers.AutoModelForCausalLM.from_pretrained("/mnt/large_hdd/Llama-2-7b-chat-hf/", torch_dtype=torch.bfloat16,config=configuration).eval().cuda()
outputs = []
...
inputs = tokenizer(prompts, add_special_tokens=False, padding=False, truncation=True, return_tensors='pt')
